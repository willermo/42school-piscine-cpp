...
/*   Updated: 2023/12/20 12:33:53 by doriani          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
main(void) {
    ...
    while (true) {
        ...
        std::cout << BLUE_U << "It's " << player->getName() << "'s turn!"
                  << RESET << std::endl;
        if (dynamic_cast<DiamondTrap *>(player) != NULL)
            std::cout
                << "What do you want to do? (attack, repair, whoami, exit): ";
        else if (dynamic_cast<ScavTrap *>(player) != NULL)
            std::cout
        ...
        } else if (!action.compare("whoami")) {
            if (dynamic_cast<DiamondTrap *>(player) != NULL)
                dynamic_cast<DiamondTrap *>(player)->whoAmI();
            else {
                std::cout << PURPLE
                          << "Don't cheat! You cannot ask who you are!" << RESET
                          << std::endl;
                i--;
                continue;
            }
